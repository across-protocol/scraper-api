...
const opRebateDeposits = mockManyDepositEntities(10, {
  overrides: {
    status: "filled",
...
const referralDeposits = mockManyDepositEntities(10, {
  overrides: {
    status: "filled",
    depositorAddr: userAddress,
    sourceChainId: 1,
...
describe("GET /rewards/referrals", () => {
    ...
    await depositFixture.insertManyDeposits([
      {
        ...
        priceId: price.id,
        stickyReferralAddress: userAddress,
        bridgeFeePct: "100000000000000000", // 1%
      },
      {
        depositId: 2,
        status: "filled",
  ...
  it("200 with params 'userAddress'", async () => {
    ...
    expect(response.body.deposits[0].rewards.rate).toBe(0.3); // 0.75 * 0.4 because userAddress is referrer
    expect(response.body.deposits[1].rewards.rate).toBe(0.1); // 0.25 * 0.4 because userAddress is depositor
    expect(response.body.deposits[2].rewards.rate).toBe(0.4); // 1 * 0.4 because userAddress is both referrer and depositor
  });
